...
struct HfCandidateSelectorB0ToDPi {
  ...
  bool selectionTopol(const T1& candB0, const T2& candD, const T3& trackPi)
  {
    auto candpT = candB0.pt();
    ...
    // B0 mass cut
    if (std::abs(invMassB0ToDPi(candB0) - RecoDecay::getMassPDG(pdg::Code::kB0)) > cuts->get(pTBin, "m")) {
    ...
    // D pt
    if (candD.pt() < cuts->get(pTBin, "pT D")) {
      return false;
    }

    // D mass cut
    // if (trackPi.sign() > 0) {
    //   if (std::abs(InvMassDplus(candD) - RecoDecay::getMassPDG(pdg::Code::kDMinus)) > cuts->get(pTBin, "DeltaM")) {
    //     return false;
    //   }
    // }
  ...
  void process(aod::HfCandB0 const& candB0s, soa::Join<aod::HfCand3Prong, aod::HfSelDplusToPiKPi> const&, aod::BigTracksPID const&)
  {
    for (const auto& candB0 : candB0s) { // looping over B0 candidates

      int statusB0 = 0;

      // check if flagged as B0 → D- π+
      if (!TESTBIT(candB0.hfflag(), hf_cand_b0::DecayType::B0ToDPi)) {
      ...
      auto candD = candB0.prong0_as<soa::Join<aod::HfCand3Prong, aod::HfSelDplusToPiKPi>>();
      auto trackPi = candB0.prong1_as<aod::BigTracksPID>();

      // topological cuts
      if (!selectionTopol(candB0, candD, trackPi)) {
        hfSelB0ToDPiCandidate(statusB0);
